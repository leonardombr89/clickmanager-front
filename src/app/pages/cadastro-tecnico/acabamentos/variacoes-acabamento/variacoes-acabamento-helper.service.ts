...
export class AcabamentoVariacaoHelperService {
  constructor(private api: ApiService) {}

  carregarDadosIniciais(): Observable<{ materiais: any[]; formatos: any[] }> {
    return forkJoin({
      materiais: this.api.get<any>('api/materiais?page=0&size=500')
        .pipe(map(res => res.content || [])),
      formatos: this.api.get<any>('api/formatos?page=0&size=500')
        .pipe(map(res => res.content || [])),
    });
  }
